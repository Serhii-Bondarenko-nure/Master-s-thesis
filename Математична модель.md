...
flowchart TD
    ...
    A[Start] --> B[Generate DAG Structure]
    
    B --> B1[Define Tasks V]
    B --> B2[Define Dependencies E]
    
    B1 --> C[Assign Task Attributes]
    B2 --> D[Assign Communication Data Size]
    
    C --> C1[Compute level_i]
    C --> C2[Assign type_i]
    
    C1 --> E[Compute Task Weight W_i]
    C2 --> E
    
    E --> F[Define Platform Model]
    
    F --> F1[Define Processors P_j]
    F --> F2[Define Bandwidth B]
    
    E --> G[Compute ETC Matrix]
    F1 --> G
    ...
    H --> H1[Compute EST and FT]
    
    H1 --> I[Compute Makespan]
    H1 --> J[Compute Average Completion Time]
    H1 --> K[Compute Processor Loads]
    
    K --> L1[Compute Load Std Deviation]
    L1 --> L2[Compute Load Coefficient of Variation]
